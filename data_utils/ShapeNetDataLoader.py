...
class PartNormalDataset(Dataset):
    def __init__(self,root = './data/shapenetcore_partanno_segmentation_benchmark_v0_normal', 
        ...
        for item in self.cat:
            ...
            if split == 'trainval':
                ...
                count = round((len(fns) / 2874) * 1870)
                fns = random.sample(fns, count)
            elif split == 'test':
                fns = [fn for fn in fns if fn[0:-4] in test_ids]
                #print('test: ', len(fns))
            else:
    ...
    def __getitem__(self, index):
        #print('Index in Shapenet - ', index)
        #print('shapenet cache size - ', len(self.cache))
        if index in self.cache:
            ppoint_set, cls, seg = self.cache[index]
            #print('inside cache shapenet - ', len(self.datapath))
...
class ACDSelfSupDataset(Dataset):
    def __init__(self, root = '/srv/data2/mgadelha/ShapeNetACD/', 
                 ...
                 k_shot=-1, exclude_fns=[], splits=None, use_val=False, prefetch=False):
        '''
        ...
        #     print(cat, self.seg_classes[cat])
        self.cache = {}  # from index to (point_set, cls, seg) tuple
        self.cache_size = len(self.datapath)
        ...
        if self.prefetch:
            ...
            for index, value in enumerate(self.datapath):
                ...
                choice = np.random.choice(len(seg), self.npoints, replace=True)
                chamfer_points = point_set[:, :]
                point_set = point_set[choice, :]  # resample
                seg = seg[choice]
                self.Labels.append(seg)
                self.Points.append(point_set)
                self.AllPoints.append(chamfer_points)
                self.Classlabels.append(cls)
    ...
    def __getitem__(self, index):
        ...
        #print('Index - ', index)
        #index = int(np.random.choice(len(self.datapath), 1)[0])
        #print('ACD cache size- ', len(self.cache))
        if self.prefetch:
            return self.Points[index], self.AllPoints[index], self.Classlabels[index], self.Labels[index]

        if index in self.cache:
            #print('inside ACD cache - ', index)
            ppoint_set, cls, seg = self.cache[index]
            #point_set, cls, seg = self.cache[index]
        else:
            fn = self.datapath[index]
        ...
        return point_set, chamfer_points, cls, seg
