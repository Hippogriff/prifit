...
from models.dgcnn import DGCNNVanillaGroupNorm
...
def main(args):
    ...
    if args.selfsup:
        ...
        if args.ss_dataset == 'dummy':
            ...
            SELFSUP_DATASET = SelfSupPartNormalDataset(root = root, npoints=args.npoint, 
                                        split='train', normal_channel=args.normal, class_choice='Airplane', 
            ...
            SELFSUP_VAL = SelfSupPartNormalDataset(root = root, npoints=args.npoint,
                                                    split='val', normal_channel=args.normal, class_choice='Airplane',
                                                    k_shot=args.n_cls_selfsup, labeled_fns=labeled_fns)
    ...
    if args.model == 'dgcnn':
        print('using DGCNN..................')
        classifier = DGCNNVanillaGroupNorm(num_classes=16, k=20, convex_loss=args.include_convex_loss, if_acd_loss=False, alpha=args.alpha, beta=args.beta, quantile=args.quantile, mode=1, intersect=args.include_intersect_loss, entropy_loss=args.include_entropy_loss, msc_iterations=args.msc_iterations, max_num_clusters=args.max_num_clusters, reconstruction=False).cuda()
    else:
        classifier = MODEL.get_model(num_part, normal_channel=args.normal).cuda()
    ...
    for epoch in range(start_epoch,args.epoch):
        ...
        for i in tqdm(list(range(num_iters)), total=num_iters, smoothing=0.9):
            ...
            out, loss_self_sup = classifier.forward(points, chamfer_points)
            #print(loss_self_sup)
