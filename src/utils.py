...
def chamfer_distance_single_shape(pred, gt, one_side=False, sqrt=False, reduce=True):
    ...
    :return:
    """
    # TODO Add kd-tree based chamfer distance implementation for faster processing!
    if isinstance(pred, np.ndarray):
        pred = Variable(torch.from_numpy(pred.astype(np.float32))).cuda()
...
def chamfer_distance_kdtree(source_points, target_points, sqrt=False):
    batch_size = source_points.shape[0]
    source_numpy = source_points.data.cpu().numpy()
    target_numpy = target_points.data.cpu().numpy()

    distances = []
    for b in range(batch_size):
        kdtree = KDTree(source_numpy[b])
        _, idx = kdtree.query(target_numpy[b], 1)
        dist_st = torch.sum((target_points[b] - source_points[b][idx[:, 0]]) ** 2, 1)

        kdtree = KDTree(target_numpy[b])
        _, idx = kdtree.query(source_numpy[b], 1)
        dist_ts = torch.sum((source_points[b] - target_points[b][idx[:, 0]]) ** 2, 1)
        if sqrt:
            dist_st = torch.sqrt(dist_st)
            dist_ts = torch.sqrt(dist_ts)
        distance = (torch.mean(dist_st) + torch.mean(dist_ts)) / 2.0
        distances.append(distance)
    distances = torch.stack(distances).mean()
    return distances


def analytic_chamfer_distance(ellipsoid_params_batch, source_points, target_points, cuboid=False):
    """

    @param ellipsoid_params_batch:
    @param source_points:
    @param target_points:
    @return:
    """
    from convex_loss import compute_sdf_ellipsoids_batch, compute_sdf_cuboid_batch
    batch_size = target_points.shape[0]

    target_numpy = target_points.data.cpu().numpy()
    if cuboid:
        sdfs = compute_sdf_cuboid_batch(target_points, ellipsoid_params_batch)
    else:
        sdfs = compute_sdf_ellipsoids_batch(target_points, ellipsoid_params_batch)
    distances = []
    for b in range(batch_size):
        if torch.is_tensor(source_points[b]):
            source_numpy = source_points[b].data.cpu().numpy()
        else:
            continue

        sdf = sdfs[b]
        # we want to select the ellipsoid with smallest distance from the point
        # in unsigned way.
        sdf_ts = torch.abs(torch.stack(sdf, 1))
        sdf_ts = torch.min(sdf_ts, 1)[0] ** 2

        kdtree = KDTree(target_numpy[b])
        _, idx = kdtree.query(source_numpy, 1)

        dist_st = torch.sum((source_points[b] - target_points[b][idx[:, 0]]) ** 2, 1)

        distance = (torch.mean(dist_st) + torch.mean(sdf_ts)) / 2.0
        distances.append(distance)
