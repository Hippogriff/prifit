...
class MeanShift:
    ...
    def mean_shift(self, X, num_samples, quantile, iterations, kernel_type="gaussian", bw=None, eff=False):
        """
        ...
        calculation
        :param X: input, N x d
        :param num_samples: number of samples used while computing the bandwidth
        ...
        if eff:
            random_samples = np.random.choice(X.shape[0], X.shape[0] // 2, replace=False)
            X_seed = X[random_samples]
            new_X, _ = self.mean_shift_eff_(X, X_seed, b=bw, iterations=iterations, kernel_type=kernel_type)
            with torch.no_grad():
                # TODO, this might be: self.nms(new_X, X, b=bw)
                _, indices, new_labels = self.nms(new_X, new_X, b=bw)
        else:
    ...
    def mean_shift_eff_(self, X, X_seed, b, iterations=10, kernel_type="gaussian"):
        """
        Differentiable mean shift clustering.
        X are assumed to lie on the hyper shphere, and thus are normalized
        to have unit norm. This is done for computational
        ...
        for i in range(iterations):
            if kernel_type == "gaussian":
                dist = X_seed @ torch.transpose(X, 1, 0)

                # TODO Normalization is still remaining.
                K = guard_exp(dist / (b ** 2))
            else:
                # epanechnikov
                dist = 2.0 - 2.0 * X_seed @ torch.transpose(X, 1, 0)
                dist = 3 / 4 * (1 - dist / (b ** 2))
                K = torch.nn.functional.relu(dist)
            ...
            # re-normalize it to lie on unit hyper-sphere.
            X_seed = X_seed / torch.norm(X_seed, dim=1, p=2, keepdim=True)
        # new_X: center of the clusters
        return X_seed, X
    ...
    def gaurd_mean_shift(self, embedding,  quantile, iterations, kernel_type="gaussian"):
        """
        ...
        """
        while True:
            center, bandwidth, cluster_ids = self.ms.mean_shift(
    ...
    def membership(self, centers, X, bandwidth):
        """
        returns per point membership
        :param centers: cluster centers of shape K, D
        :param per point embedding, N x D
        """

        similarity = centers @ X.T
        similarity = similarity / (bandwidth ** 2)
        # we don't want to back prop gradients through max node separately,
        # as this is not mathematically equivalent to membership function in backward pass.
        # max here is not used for numerical stability.
        similarity = similarity - similarity.max().detach()
        # shape K x N, VM kernel
        kernel = guard_exp(similarity)
        # shape K x N, normalize the kernel across different cluster centers
        mem = kernel / (torch.unsqueeze(torch.sum(kernel, 0), 0))
        return mem

    def oldmembership(self, centers, X, bandwidth):
        """
        returns per point membership
        :param centers: cluster centers of shape K, D
        :param per point embedding, N x D
        ...
        similarity = centers @ X.T
        # shape K x N, VM kernel
        kernel = guard_exp(similarity / (bandwidth ** 2) / 2)
        # shape K x N, normalize the kernel across different cluster centers
        mem = kernel / (torch.unsqueeze(torch.sum(kernel, 0), 0))
        return mem
